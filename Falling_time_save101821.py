...
instr.set_mode('register', values=[14,17,16,15]*16)
...
for i in range(10):
    ...
    seq += [fs.Panel4(clk=True, even_value=0.5, odd_value=0.6)] # 16
    
    seq += [fs.Panel4(clk=False, even_value=0.5, odd_value=0.8)]
    seq += [fs.Panel4(clk=True, even_value=0.5, odd_value=0.8)] # 15
    
    seq += [fs.Wait(value=100e3, precision='1ms')]
