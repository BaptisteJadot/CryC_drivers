# -*- coding: utf-8 -*-
"""
Created on Tue Nov  9 16:38:52 2021
...
scope.set_horizontal(sec_per_div=200e-6, sec_offset=500e-6)
scope.wait_opc()

## PARAMETERS
default_values = [0.5]*64
default_values[14] = 1.
ramp = np.linspace(0.5, 1.5, int(100*AWG_SAMPLING_RATE_MHz))
clk_rate_MHz = 0.16
data_to_clk_us = 1 / clk_rate_MHz * 0.5
do_save = False
fname = "voltage_offset_14_vs_all_1.h5"

## PREPARE PLOT
plt.figure(450);plt.clf();plt.ion()

## RUN
data = []
for i in range(2):
    val = [default_values[(i + k) % 64] for k in range(64)]
    Veven, Vodd, CLK = build_waveforms(val, clk_rate_MHz, data_to_clk_us, current_index=(i - 1) % 64, current_val=0.5)
    if i % 2 == 0:
        Veven = np.concatenate((Veven, ramp))
        Vodd = np.concatenate((Vodd, np.zeros_like(ramp)))
        CLK = np.concatenate((CLK, np.zeros_like(ramp)))
    else:
        Veven = np.concatenate((Veven, np.zeros_like(ramp)))
        Vodd = np.concatenate((Vodd, ramp))
        CLK = np.concatenate((CLK, np.zeros_like(ramp)))
    ...
    print(instr.SPI_read(0xA0, 1)[0])
    
    # get traces
    if scope.instr.query("ACQ:STAT?") == "COMP":
        t, traces = scope.get_traces()
        data += [traces["CH1"]]
        print(f"Got {traces['CH1'].shape} pts")
        # plt.plot(t, traces["CH1"], label=f"{i}")
        # plt.legend()    
