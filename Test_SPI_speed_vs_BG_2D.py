...
VBGNC = np.linspace(-0.2,1.2,15)
VBGPC = np.linspace(0.2,-1.2,15)
N = 10
max_speed = np.zeros((len(VBGNC), len(VBGPC), N))

for i, VBGi in enumerate(VBGNC):
    for j, VBGj in enumerate(VBGPC):
        DAC_val['VBGNC'] = VBGi
        DAC_val['VBGPC'] = VBGj
        instr.update_DAC(DAC_val)
        ...
        for k in range(N):
            for speed_ticks in range(start,100,-1):
                ...
                if Nerr > 0:
                    break
            max_speed[i, j, k] = np.NaN if speed_ticks == start else speed_ticks
            print(f'[{VBGi:.1f}V, {VBGj:.1f}V] : max speed {40/max_speed[i,j,k]*1000:.2f} kHz ({max_speed[i,j,k]} ticks).')
...
with h5.File('''D:\Baptiste\CIR7\\4K\SPI_speed_vs_BG2D.h5''', 'a') as f:
    i = 0
    while f'data_{i}' in f.keys():
        i += 1
    grp = f.create_group(f'data_{i}')
    grp.create_dataset('VBGNC', data=VBGNC)
    grp.create_dataset('VBGPC', data=VBGPC)
