...
def build_waveforms(values=[0.5]*64, current_index=63, current_val=0.):
    # build lists
...
if obs_addr % 2 == 0:
    instr.SPI_write(0x70, [0x00, 0x00, 0x80 | (obs_addr//2)%32, 0x00])
else:
    instr.SPI_write(0x70, [0x00, 0x00, 0x00, 0x80 | (obs_addr//2)%32])
...
seq += [fs.Panel4(clk=True, even_value=Vsweep[0], odd_value=0.5)]
seq += [fs.Wait(value=100, precision='1us')]

for Vi in Vsweep:
    seq += [fs.Panel4(clk=True, even_value=Vi, odd_value=0.5)]
    seq += [fs.ADC_get(channel=2 if (obs_addr % 2) == 0 else 3)]
    seq += [fs.JumpFor(target=len(seq)-1, count=N0-1)] # read 50 times
    
seq += [fs.Panel4(clk=True, even_value=0.5, odd_value=Vsweep[0])]
seq += [fs.Wait(value=100, precision='1us')]

for Vi in Vsweep:
    seq += [fs.Panel4(clk=True, even_value=0.5, odd_value=Vi)]
    seq += [fs.ADC_get(channel=2 if (obs_addr % 2) == 0 else 3)]
    seq += [fs.JumpFor(target=len(seq)-1, count=N0-1)] # read 50 times
...
## SAVE DATA
fname = f'''D:\Baptiste\CIR7\\4K\\Crosstalk_V{obs_addr}_vs_all_muxmat_1.h5'''
